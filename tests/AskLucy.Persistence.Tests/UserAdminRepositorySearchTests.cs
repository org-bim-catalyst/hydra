...
public sealed class UserAdminRepositorySearchTests(PersistenceTestFixture fixture)
{
    ...
    public async Task SearchAsync_ShouldFilterByPartialEmailOrName_SortAndPaginate_Correctly()
    {
        ...
        var searchResult = await repository.SearchAsync(search: $"jane.doe-{suffix}", sortBy: "email", sortDescending: false, page: 1, pageSize: 20, cancellationToken: TestContext.Current.CancellationToken);
        ...
        var sortedResult = await repository.SearchAsync(
            search: suffix, sortBy: "createdAtUtc", sortDescending: true, page: 1, pageSize: 20, cancellationToken: TestContext.Current.CancellationToken);
        sortedResult.TotalCount.Should().Be(3);
        sortedResult.Items.Select(u => u.FirstName).Should().ContainInOrder("Alex", "John", "Jane");

        var page2 = await repository.SearchAsync(search: suffix, sortBy: "createdAtUtc", sortDescending: true, page: 2, pageSize: 2, cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task SearchAsync_ShouldExcludeSoftDeletedUsers()
    {
        ...
        var result = await repository.SearchAsync(search: suffix, sortBy: "email", sortDescending: false, page: 1, pageSize: 20, cancellationToken: TestContext.Current.CancellationToken);
