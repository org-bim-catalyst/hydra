...
**Alternatives considered**: audit trail alone (above); a separate `TurnOutcome` table keyed by message id (a join and a new aggregate buying nothing the column does not, and it reintroduces the two-write consistency problem FR-004d exists to prevent); in-memory cache only (does not survive reload, fails SC-001c).
