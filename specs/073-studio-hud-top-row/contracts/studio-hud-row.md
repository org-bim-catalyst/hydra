...
| B2 | Shows exactly, on one line: a confidence shield · `siteName` (`subtitle2`, weight 600 — bold like the project title, `noWrap`, ellipsis). Nothing else on the card (FR-006, FR-007). The level and its reason are in the shield's tooltip: the level's label, then `Outline: {sourceDetail}` once a boundary is drawn, or the `__LOCATION__` event's `confidenceReason` before that. The shield is the one part of the card with `pointer-events: auto` (the rest lets drags through to the map) and is focusable (`tabIndex=0`, `role="img"`, `aria-label` = the level's label). Amended twice on 2026-09-25: first to one line (icon · label · divider · name) to match the weather card, then — at the user's request — the label and divider moved into the tooltip and the name made bold. |
...
| B5 | `maxWidth: 360` (was 260 while the card was two lines). |
