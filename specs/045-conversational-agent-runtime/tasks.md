...
> **More live-production findings (2026-09-11), not numbered tasks.** (1) `ResolveLocationCapability`'s JSON output dropped `locationType`/`viewport` entirely — `ILocationResolutionService` resolved them correctly (specs/038), but the capability boundary introduced in Phase 6 never forwarded them, so the viewer's own auto-zoom (`ViewerSurface.tsx`) always fell back to a wide default. Fixed by including both fields in the output and `StructuredPayloadExtractor`'s reconstruction. (2) **That fix exposed a second, previously-masked bug**: `LocateAPlaceFlow`'s step 2 (`adjust_viewer_focus`, hardcoded `direction:"in"`, documented in the Phase 6 note above as standing in for "focus the viewer") was *always* a forced additional zoom-in stacked on top of step 1's own automatic recentre. While (1) was broken, step 1's own zoom was too wide, so this second zoom-in happened to land somewhere reasonable by coincidence; once (1) was fixed, step 1 alone already frames the site correctly, and this step's zoom-in then over-zoomed on top of it — live-tested as "zoomed twice" and "too tight to see the boundary while rotating." Fixed by **removing this flow step entirely** (`locate_a_place` is now two steps, not three) rather than special-casing it — `AdjustViewerFocusCapability` itself, and its availability for a direct "zoom in"/"zoom out" request, are untouched; only the flow's own forced invocation of it is gone. `contracts/capability-flow.md` and spec.md's FR-051/FR-051b are updated to match the two-step flow. **No database changes** in either fix.
