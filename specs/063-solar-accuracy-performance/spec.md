...
The correctness item is the reason this is a specification rather than a maintenance ticket. The feature reports a solar altitude computed as the sun's *true geometric* position, and separately reports sunrise and sunset times computed against the standard **−0.833°** threshold — a figure that silently bundles two unrelated physical effects, atmospheric refraction (~0.57°) and the sun's angular radius (~0.27°). Neither effect is applied to the altitude the feature displays. The two figures consequently disagree by about eight tenths of a degree at exactly the low-sun moments the feature exists to explain, and a user checking one against the other finds the tool inconsistent with itself.

Resolving this requires more than adding a correction to the displayed figure. Measurement during planning established that a refraction correction alone leaves a residual of 0.34°–0.61° that *varies* by site and season, because the −0.833° threshold is itself an approximation applied to a position evaluated once per day. The release instead separates the two bundled effects and uses each where it belongs: refraction is applied to the reported altitude, and the sun's radius defines what "the sun has risen" means. Rise and set are then solved for directly rather than approximated. The reported altitude at sunrise becomes a constant −0.267° everywhere — the sun's radius, and nothing else — which is both explainable in one sentence and verifiable as a single number.
...
- Q: Numerical checking during planning showed the original premise was wrong — adding a refraction correction alone does **not** make the altitude read zero at the computed sunrise. What is the actual target? → A: Make the two calculations genuinely self-consistent rather than merely less inconsistent. The standard −0.833° rise/set threshold bundles two distinct physical effects: atmospheric refraction (~0.57°) and the sun's angular radius (~0.27°). Sunrise is the moment the sun's **upper edge** appears, so the **centre** — the point whose altitude is reported — is legitimately still below the horizon at that instant. Reporting zero there would be wrong, not right. The release therefore reports the refraction-corrected altitude of the sun's centre, and computes rise and set by solving for the instant the refraction-corrected **upper edge** reaches the horizon. At sunrise and sunset the reported altitude is then exactly **−0.267°** — the sun's angular radius — at every location and date, by construction. Measured across five sites and four dates spanning the tropics to Reykjavík, this holds to within 0.0001°. A constant with a one-line explanation is both more honest and easier to verify than a zero that cannot be achieved.
- Q: Does solving for rise and set instead of approximating them change the reported times? → A: Yes, and this supersedes the earlier assumption that they would be unchanged. Measured shifts are 12–55 s in the tropics, up to 77 s at mid latitude, and up to 172 s at Reykjavík and Tromsø. The shift is the removal of existing error, not the introduction of new error: the current implementation evaluates the sun's declination and equation of time once at local noon and applies them to the whole day, whereas solving evaluates the sun's actual position at the instant in question. The new times must therefore be validated against published reference values rather than against the current release's output, and the current release must be treated as the less accurate of the two wherever they disagree.
...
0. **Given** any site and date, **When** the user compares the altitude figure against the rise and set times, **Then** the two are derived from one definition of the horizon and cannot disagree about which side of it the sun is on.
1. **Given** a site and date with a normal sunrise, **When** the user sets the time to the sunrise the figures panel reports, **Then** the reported solar altitude reads −0.27° within the stated tolerance.
2. **Given** a site and date with a normal sunset, **When** the user sets the time to the reported sunset, **Then** the reported solar altitude reads −0.27° within the stated tolerance.
2a. **Given** any other site or date with a normal sunrise, **When** the same check is repeated, **Then** the reading is the same −0.27°, because it is the sun's angular radius and not a site-dependent residual.
...
- **FR-002**: At the sunrise and sunset instants the system itself computes, the altitude the system reports MUST equal the negative of the sun's angular radius — −0.267° — within the stated position tolerance, at every location and date. This value MUST be constant rather than varying by site or season.
- **FR-002a**: The system MUST determine sunrise and sunset by solving for the instant at which the refraction-corrected upper edge of the sun reaches the horizon, rather than by applying a fixed threshold to a position evaluated once per day.
...
- **FR-004**: The system MUST continue to state polar conditions — sun never rises, sun never sets — plainly, determined against the same upper-edge definition used for rise and set so that the three can never disagree about whether the sun rose.
- **FR-004a**: Reported sunrise and sunset times MUST be validated against published reference values, not against the current release's output. Where the two disagree, the current release MUST be treated as the less accurate.
...
- **SC-001**: At the sunrise and sunset times the feature reports, the altitude the feature reports reads −0.267° within the stated position tolerance, for every test location including at least one inside the polar circles, and the value is the same at every location and date rather than varying with either.
- **SC-002**: Reported altitude agrees with published reference values for the refraction-corrected quantity within the stated position tolerance, across a full day, at every test location.
- **SC-003**: Sunrise and sunset times agree with published reference values within the stated rise/set tolerance at every test location and date. **Met**: worst case 40.2 s against NOAA's published annual table across 18 rows spanning Singapore, Dubai, London, Reykjavík and Tromsø, asserted in `daySummary.test.ts`.

  The pre-implementation estimate in this line — "12–55 s in the tropics and up to 172 s at high latitude" — was wrong by roughly an order of magnitude and is corrected here. **Measured after implementation**: 2.1–2.5 s in the tropics, 3.5–4.1 s at mid latitude, 5.2–17.4 s at high latitude. The estimate assumed splitting refraction from the semidiameter would move the rise/set threshold by the full semidiameter (0.267°). It does not: the derived threshold is −0.8416° against the bundled −0.833° it replaces, a difference of 0.0086°, because refraction at the horizon (≈0.575°) plus the semidiameter (0.2667°) very nearly *is* the 0.833° the old constant bundled. That is the substantive finding: the split barely moves rise and set, and its real value is elsewhere — it gives the *reported altitude at sunrise* a single well-defined value (SC-001) instead of one that drifted with latitude and date. Every measured difference still moves the reported time toward the published reference.
