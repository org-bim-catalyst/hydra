...
- [X] T010 Generate the EF Core migration: new `UserChat`/`Message` columns, new `Attachments`/`Citations` tables, new indexes, and a SQL Server full-text catalog + full-text index on `Message.Content` and `UserChat.Title` (research.md Topic 5) via `dotnet ef migrations add AddConversationManagement -p src/AskLucy.Persistence -s src/AskLucy.Web`; verify `Down()` is reversible or document why not (constitution §5 Migrations) (depends on T006–T009) — **split into two migrations after real-world testing**: `CREATE FULLTEXT CATALOG`/`INDEX` cannot run inside a transaction, but EF wraps a whole migration in one by default, so the FTS DDL was moved to its own follow-up migration, `AddConversationFullTextSearch` (`suppressTransaction: true`), per EF Core's own tooling warning. Verified applying cleanly end-to-end against a real (non-LocalDB) SQL Server instance, both a dev instance and production. Confirmed separately that SQL Server LocalDB/User Instances do not support Full-Text Search at all (`Cannot use full-text search in user instance`, error 9982) — `AddConversationManagement` alone still applies and the feature works fully except message-content/title search on such instances
...
- [ ] T078 Run the full `quickstart.md` validation guide (all 7 scenarios) end-to-end against a fresh local environment and record results — **partially de-risked, still not run**: the migrations (`AddConversationManagement` + `AddConversationFullTextSearch`) have since been verified applying cleanly against both a real local SQL Server and production (see T010's note), which was the highest-risk untested part of this feature. The actual user-facing scenarios (search/filter/sort, pin/archive/duplicate/clear/export/permanent-delete through the UI) still haven't been walked end-to-end — that requires a running frontend + a real OpenAI key, which wasn't available in the sandbox this feature was built in
