...
- **Backend tests**: `tests/AskLucy.Domain.Tests/`, `tests/AskLucy.Application.Tests/`, `tests/AskLucy.Infrastructure.Tests/`, `tests/AskLucy.Persistence.Tests/`, `tests/AskLucy.Web.Tests/`, `tests/AskLucy.E2E.Tests/`
- **Frontend**: `src/AskLucy.Web/ClientApp/src/`, `src/AskLucy.Web/ClientApp/tests/` (originally `frontend/`, moved when the frontend and backend were merged into one project — see later notes)
...
- [X] T001 Add `src/AskLucy.Domain`, `src/AskLucy.Application`, `src/AskLucy.Infrastructure`, `src/AskLucy.Persistence`, `src/AskLucy.Web` projects (and matching `tests/AskLucy.*.Tests` projects) to `Ask Lucy.sln`, referencing each other per the Dependency Rule in `plan.md` § Constitution Check; leave the existing `Ask Lucy/` project untouched.
...
- [X] T008 Configure `AskLucyDbContext` in `src/AskLucy.Persistence/AskLucyDbContext.cs`, extending `IdentityDbContext<ApplicationUser>`, wired to the `DefaultConnection` connection string (unchanged database).
...
- [X] T014 [P] Implement centralized Problem Details exception-handling middleware in `src/AskLucy.Web/Middleware/ProblemDetailsMiddleware.cs` per `contracts/api-v1.md` § Error format.
- [X] T015 [P] Configure Serilog structured logging and a correlation-ID middleware in `src/AskLucy.Web/Program.cs` / `src/AskLucy.Web/Middleware/CorrelationIdMiddleware.cs`.
- [X] T016 [P] Configure `Microsoft.AspNetCore.RateLimiting` with a per-user partitioned fixed-window limiter (20 req/min regular, 100 req/min Administrator/Super User) in `src/AskLucy.Web/Program.cs`, per `research.md` Topic 3.
...
- [X] T019 [P] Configure OpenAPI/Swagger generation in `src/AskLucy.Web/Program.cs` reflecting `contracts/api-v1.md`. *(Uses .NET 10's built-in `Microsoft.AspNetCore.OpenApi` rather than Swashbuckle — see architecture note below.)*
- [X] T020 [P] Configure an explicit CORS allow-list (frontend origin(s) only, replacing today's wildcard) in `src/AskLucy.Web/Program.cs`, per `research.md` Topic 7.
...
- [X] T030 [P] [US1] Contract tests for `/api/v1/auth/2fa/{enable,disable,recovery-codes}` in `tests/AskLucy.Web.Tests/Auth/TwoFactorManagementTests.cs` (401-without-token for all three endpoints).
...
- [X] T032 [US1] Implement the SSE streaming endpoint `POST /api/v1/ai/chat` in `src/AskLucy.Web/Controllers/v1/AiController.cs` (depends on T031). *(Controller-based, not minimal-API `Endpoints.cs`, matching the `-controllers` WebAPI template scaffolded in T001.)*
- [X] T033 [P] [US1] Implement `TranslateCommand`/handler + `POST /api/v1/ai/translate` endpoint in `src/AskLucy.Application/Ai/Commands/Translate/` and `src/AskLucy.Web/Controllers/v1/AiController.cs`.
- [X] T034 [P] [US1] Implement `GenerateImageCommand`/handler + `POST /api/v1/ai/images` endpoint in `src/AskLucy.Application/Ai/Commands/GenerateImage/` and `src/AskLucy.Web/Controllers/v1/AiController.cs`.
- [X] T035 [P] [US1] Implement `TranscribeAudioCommand`/handler + `POST /api/v1/ai/transcriptions` endpoint in `src/AskLucy.Application/Ai/Commands/Transcribe/` and `src/AskLucy.Web/Controllers/v1/AiController.cs`.
- [X] T036 [US1] Implement `CreateUserChatCommand` and `GetMyUserChatsQuery` handlers in `src/AskLucy.Application/Chats/`.
- [X] T037 [US1] Implement `RenameUserChatCommand` and `DeleteUserChatCommand` (soft delete) handlers in `src/AskLucy.Application/Chats/Commands/`, wired to `PATCH`/`DELETE /api/v1/chats/{id}` in `src/AskLucy.Web/Controllers/v1/ChatsController.cs` (FR-033).
- [X] T038 [US1] Implement `Register`, `Login`, `Login2fa`, `Refresh`, `Logout`, and external-login-callback handlers in `src/AskLucy.Application/Authentication/`, reusing existing `UserManager`/`SignInManager` behavior (via the new `IIdentityService`), and their endpoints in `src/AskLucy.Web/Controllers/v1/AuthController.cs`. Also fixes the legacy email-confirmation template race condition (T064's original scope) by rendering the email in-memory per request from the start, rather than introducing the bug and fixing it later.
- [X] T039 [US1] Implement TOTP enable, disable, and recovery-code-generation handlers in `src/AskLucy.Application/Authentication/Commands/TwoFactor/`, wired to `/api/v1/auth/2fa/{enable,disable,recovery-codes}` in `src/AskLucy.Web/Controllers/v1/AuthController.cs` (FR-011).
- [X] T040 [US1] Implement `GET/PATCH /api/v1/users/me`, avatar upload (`PUT /api/v1/users/me/avatar`), and the signed-URL avatar download endpoint in `src/AskLucy.Web/Controllers/v1/UsersController.cs`, per `research.md` Topic 6. *(Download endpoint ended up as `GET /api/v1/users/{userId}/avatar?exp=&sig=` rather than literally `/me/avatar` — a signed URL must be usable without an Authorization header, e.g. from an `<img>` tag, so it can't rely on "me" resolving from a bearer token; the upload endpoint remains at `/me/avatar`.)*
...
- [~] T047 [US1] Execute the rehearsed `UserChats`/avatar data migration (T010, T011) against a restored production copy and verify row-for-row parity, per `quickstart.md` § 6. **`UserChats` int→GUID PK conversion done and verified** (baseline `InitialCreate` row inserted, `RefreshTokens` created, `UserChats` converted with row count confirmed unchanged at 4 rows) against the restored copy, which has since been promoted to be the production database directly rather than replayed to a separate live instance. **Avatar migration still outstanding**: `AvatarFileName` was added as a nullable column, but the `ProfilePicture`-BLOB-to-file data-fix code (T011) still doesn't exist, so `ProfilePicture` has not been dropped yet.
...
- [X] T048 [P] [US2] Contract tests asserting `401` for unauthenticated requests to all four AI endpoints in `tests/AskLucy.Web.Tests/Ai/AnonymousAccessTests.cs`.
- [X] T049 [US2] Apply and verify `[Authorize]` on every AI endpoint in `src/AskLucy.Web/Controllers/v1/AiController.cs` (class-level `[Authorize]`, applied from T031 onward rather than as a separate retrofit step — auth-required-by-default was built in from the start, not bolted on afterward). Verified passing via T048's tests.
...
- [X] T056 [US3] Replace raw `ApplicationUser` serialization in the admin users list endpoint with the `UserAdminDto` projection from T055 — new `GetAllUsersQuery`/handler in `src/AskLucy.Application/Users/Queries/GetAllUsers/`, wired to `GET /api/v1/users` in `src/AskLucy.Web/Controllers/v1/UsersController.cs`. (No legacy raw-entity endpoint ever existed in the new codebase to "replace" — this is a from-scratch, DTO-only implementation, which structurally can't regress into the legacy exposure.)
- [X] T057 [US3] Implement `UpdateUserCommand`/handler (explicit allow-listed fields only — no client-supplied entity persisted as-is) in `src/AskLucy.Application/Users/Commands/UpdateUser/`, wired to `PATCH /api/v1/users/{userId}` in `src/AskLucy.Web/Controllers/v1/UsersController.cs` — closes the legacy overposting/mass-assignment vulnerability from `spec.md` § Gap Analysis by construction (the request DTO has no other properties to bind).
...
- [X] T058 [P] [US4] Contract tests: non-admin gets `403` on every admin endpoint, admin passes authorization, in `tests/AskLucy.Web.Tests/Admin/RoleAuthorizationTests.cs`. Uses a self-signed test JWT (`TestJwtFactory`) so the role-check *policy* is genuinely exercised without a live database — the admin case is verified to pass authorization (never 401/403) even though the request can't succeed end-to-end without a real user in a real database.
- [X] T059 [US4] Implement and apply the `AdministratorOrSuperUser` authorization policy (already registered in `Program.cs` during Phase 2) to the admin endpoints in `src/AskLucy.Web/Controllers/v1/UsersController.cs` (`GET /api/v1/users`, `PATCH /api/v1/users/{userId}`), replacing the legacy UI-only role check.
...
- [X] T065 Decommission the legacy `Ask Lucy/` project and its `Ask Lucy.sln` reference once every checkpoint above has confirmed full parity in production, per `spec.md` § Migration Strategy step 6. The legacy `Ask Lucy.sln` project reference was already removed; the `Ask Lucy/` project directory has now been removed from the repository (1,203 tracked files). Note T047's avatar migration is still partially outstanding (see above) — the user made the call to decommission ahead of that being fully closed out.
...
Task: "Implement TranslateCommand/handler + endpoint in src/AskLucy.Application/Ai/Commands/Translate/ and src/AskLucy.Web/Endpoints/AiEndpoints.cs"
Task: "Implement GenerateImageCommand/handler + endpoint in src/AskLucy.Application/Ai/Commands/GenerateImage/ and src/AskLucy.Web/Endpoints/AiEndpoints.cs"
Task: "Implement TranscribeAudioCommand/handler + endpoint in src/AskLucy.Application/Ai/Commands/Transcribe/ and src/AskLucy.Web/Endpoints/AiEndpoints.cs"
```
...
- No task in this list introduces Docker, an Azure hosting cutover, credential rotation, RAG, agents, or any other Phase-1+ capability — all are out of scope per `spec.md`, consistent with `plan.md` § Complexity Tracking. T070/T071 document the Docker/credential deferrals; they do not remediate them. **Exception**: T078 (Phase 10) introduces a persisted chat-history baseline, an explicit, recorded override of FR-026's original scope — see spec.md's 2026-07-28 Clarifications — not an oversight of this rule.

---

## Phase 9: Convergence

- [X] T073 Implement real server-side OAuth challenge/callback endpoints for Google and Facebook (verifying the provider ID-token/authorization-code server-side — signature, audience, issuer) and remove the insecure resolve-by-email auto-link in `IdentityService.ValidateExternalLoginAsync` (`src/AskLucy.Persistence/Identity/IdentityService.cs`), which currently lets any anonymous caller mint a session for an arbitrary account by POSTing a victim's email to `POST /api/v1/auth/external` with no verification against Google/Facebook at all; wire the existing Google/Facebook buttons in `frontend/src/features/auth/pages/LoginPage.tsx` (which currently link to a nonexistent `GET /api/v1/auth/external/{provider}` route) to the new flow, per FR-010 (contradicts). *(Implemented as a full redesign, not a patch: `IIdentityService.ResolveExternalLoginAsync` (renamed from `ValidateExternalLoginAsync`) now takes an explicit `emailVerified` flag and only ever auto-links/auto-registers by email when the provider itself asserts it's verified — never a client-supplied claim. Real Google/Facebook OAuth handlers are registered in `Program.cs` (`AddGoogle`/`AddFacebook`, conditionally — only when `Authentication:Google/Facebook` client credentials are configured, since ASP.NET Core's authentication middleware validates every request-handler scheme's options on every request, and an always-registered-but-empty ClientId would throw on every request, not just a challenge). `ExternalAuth.HandleTicketReceivedAsync` (`src/AskLucy.Web/Auth/ExternalAuth.cs`) intercepts the OAuth ticket before any cookie sign-in, resolves it via the new `ProcessExternalLoginCallbackCommand`, and redirects the browser to a new frontend page (`ExternalLoginCompletePage` at `/auth/external-complete`) with a one-time, single-use, 2-minute completion code (`IExternalLoginCodeStore`/`InMemoryExternalLoginCodeStore`) — never a token in the URL. `POST /api/v1/auth/external/complete` exchanges that code for real JWT/refresh tokens. FR-034 (external-login linking, added during `/speckit-clarify`) is implemented in the same pass: `POST /api/v1/auth/external/link-ticket` (authenticated) issues a short-lived link ticket, `GET /api/v1/auth/external/{provider}/link?ticket=` (a plain top-level navigation, which can't carry an Authorization header) consumes it and challenges with `mode=link`, and a "Link Google/Facebook" button was added to `SettingsPage`'s Connected Accounts section. Also fixed, incidentally: `EnableTwoFactor` and the new `IssueExternalLoginLinkTicket` both return a bare `string`, which ASP.NET Core serializes as `text/plain` by default (not JSON) unless `[Produces("application/json")]` is applied — found while writing this task's own tests, silently broken for `enableTwoFactor()` on the frontend before this fix. New tests: `ProcessExternalLoginCallbackCommandHandlerTests`, `CompleteExternalLoginCommandHandlerTests`, `IssueExternalLoginLinkTicketCommandHandlerTests`, `InMemoryExternalLoginCodeStoreTests`, `ExternalLoginTests` (WebAPI auth-gate/not-configured-provider boundary tests). **Cannot be executed in this environment**: an actual browser round-trip against real Google/Facebook consent screens needs real OAuth app credentials and a live deployment — same limitation as T028/T029's Playwright/2FA-continuity tests. All 82 backend tests and both frontend tests pass.)*
- [X] T074 Add a secret-delivery mechanism for the production deployment (e.g. a gitignored `appsettings.Production.json` pushed to the `site4now.net` host outside CI/CD, since the shared host has no environment-variable panel or secrets vault) so `Jwt:SigningKey`, `OpenAI:ApiKey`, `SendGrid:ApiKey`, and the real database connection string actually reach the deployed app — currently `.github/workflows/ci.yml`'s `deploy` job only publishes the secret-free `appsettings.json`/`appsettings.Development.json`, per FR-030/SC-007 (missing). *(Implemented per FR-035, added during `/speckit-clarify`: `src/AskLucy.Web/appsettings.Production.json` is gitignored (`.gitignore`), with a tracked `appsettings.Production.json.example` template documenting every required key (connection string, JWT signing key, OpenAI/SendGrid keys, Google/Facebook OAuth secrets). No code change was needed beyond that — ASP.NET Core layers `appsettings.{Environment}.json` over `appsettings.json` automatically under `ASPNETCORE_ENVIRONMENT=Production`. The manual push runbook is documented in `CONTRIBUTING.md` § Production secrets. **Not actually pushed to the live `site4now.net` server** — this environment has no access to it; that one-time manual step remains the operator's responsibility per the runbook.)*
- [X] T075 Register a dev-only console/log-based `IEmailSender` for the `Development` environment, alongside the existing `SendGridEmailSender` (`src/AskLucy.Infrastructure/Email/SendGridEmailSender.cs`), so a fresh clone can complete first registration/login without a real SendGrid key (missing). *(Implemented: new `ConsoleEmailSender` (`src/AskLucy.Infrastructure/Email/ConsoleEmailSender.cs`) logs the email instead of sending it via structured logging; `AddInfrastructure` now takes an `IHostEnvironment` and registers it only for `environment.IsDevelopment()`, `SendGridEmailSender` everywhere else (including `Testing`, unchanged). **Note**: `SendGridEmailSender` itself was later removed entirely by T076 below — the non-Development branch now registers `SmtpEmailSender` instead; this task's own dev/Development-only behavior is unaffected.)*

---

## Phase 10: Post-live-testing fixes (2026-07-28, same-day follow-up)

Found and fixed while running the app live in a browser for the first time after Phase 9, per user report.

- [X] T076 Replace SendGrid with the organization's own SMTP host (`mail.bimcatalyst.com`, STARTTLS, port 587) using MailKit, per user-supplied requirements (From addresses for transactional/support/privacy categories). *(`SendGridEmailSender`/`SendGridOptions` deleted; new `SmtpEmailSender`/`SmtpOptions` (`src/AskLucy.Infrastructure/Email/`) use MailKit's `SmtpClient` with `SecureSocketOptions.StartTls`. Only `FromTransactional` is wired to a call site today (registration/email-change confirmation, password reset, 2FA) — `FromSupport`/`FromPrivacy` are configured for when those email categories exist. `appsettings.json`'s `SendGrid` section replaced with `Smtp`; `appsettings.Production.json.example` and `README.md`/`CONTRIBUTING.md` updated to match. **Not live-tested against the real mail host** — no network path to `mail.bimcatalyst.com` from this environment; `ConsoleEmailSender` still covers local Development testing.)*
- [X] T077 Add a config-driven, dev-only Administrator seed (Super User/Administrator roles were never created on a fresh database, and ADR-0001 explicitly rejects the legacy hardcoded-seed-admin-password pattern). *(New `DevAdminSeeder` (`src/AskLucy.Web/DevSeed/DevAdminSeeder.cs`), run from `Program.cs` only when `Environment.IsDevelopment()`, wrapped in try/catch so a missing database degrades to a logged warning rather than a crashed host. Creates the "Administrator"/"Super User" roles if missing, and — only if no Administrator/Super User already exists — one admin account from `SeedAdmin:Email`/`SeedAdmin:Password` config (user-secrets/env vars, never hardcoded in source or migrations). Verified live: logging in as the seeded account returns a JWT with `role: Administrator`.)*
- [X] T078 Add persisted, ChatGPT-style chat history (chat/translation/image-generation turns), overriding FR-026's original "no persisted message/conversation-content table" constraint — an explicit, recorded scope decision (see spec.md's 2026-07-28 Clarifications, FR-026/FR-036/FR-037, and § Key Entities), not a silent change. Found live: the chat sidebar and the conversation pane were completely disconnected (sending a message never touched the sidebar; clicking a sidebar entry loaded nothing) — confirmed via legacy `app.ts` archaeology that this exact disconnection already existed in the legacy app (chat creation was always a separate, manual, title-only action there too), so it wasn't a migration regression, but the user asked to fix it now anyway. *(New `Message` domain entity (`src/AskLucy.Domain/Chats/Message.cs`: Role User/Assistant, Kind Text/Image/Translation, Content, optional SourceText, standard audit columns, immutable/append-only) + EF config + `AddMessages` migration. New `AppendMessageCommand` (persists one turn, ownership-checked) composed at the controller level in `AiController` — chosen deliberately over threading persistence into `SendChatMessageCommand`/`TranslateCommand`/`GenerateImageCommand` themselves, so those commands' existing, already-tested behavior is untouched. New `GetChatMessagesQuery` + `GET /api/v1/chats/{id}/messages`. Frontend: `ChatPage` now owns `selectedChatId`; `ChatSidebar` entries are clickable/selectable and the "New chat" button just clears the selection (no more eagerly-created empty "New chat" rows — creation is deferred to the first message actually sent, via `useChatStream`'s `ensureChatId`). The conversation pane (`ConversationView`, extracted from `ChatPage`) is remounted via `key` only on *explicit* navigation (picking a different chat / starting a new one) — auto-creating a chat mid-send updates `chatId` without remounting, so an in-flight stream is never interrupted by its own id arriving. Image-kind history is rendered by encoding it as a Markdown image (`![prompt](url)`) into the existing `ReactMarkdown` pipeline — no `dangerouslySetInnerHTML`, no new rendering path. New tests: `AppendMessageCommandHandlerTests`, `GetChatMessagesQueryHandlerTests` (ownership scoping). All 86 backend tests and 3 frontend tests pass; live-verified via Playwright — send, reload, image/translation turns all round-trip correctly.)*

**Also found and fixed in this phase, incidentally:**
- A real streaming bug (unrelated to any task above): `frontend/src/features/chat/api/aiApi.ts`'s SSE parser called `.trim()` on every streamed chunk instead of stripping only the single SSE-protocol-mandated leading space, silently eating the leading space GPT puts on nearly every word token and running every streamed reply together with no spaces at all. Fixed; regression test added (`aiApi.test.ts`).
- `useVoiceRecognition.ts`'s error handling was completely silent (`onerror: () => setIsListening(false)`) — a permission denial, missing microphone, network error, or the browser never starting at all all looked identical to "nothing happened." Added error messages surfaced via a Snackbar in `ChatComposer.tsx`, plus a 15s watchdog for the "never even started" case (verified live to correctly catch a stuck/no-permission-decision state). **Confirmed working end-to-end by the user afterward** in a real browser — the "stays quiet on a real working mic" path couldn't be validated in this sandbox (headless Chromium here never fires speech recognition's `onstart` event at all, granted permission or not) but is now confirmed fine in practice. The root cause of the original complaint was most likely the transient network-level block surfaced by the `'network'` error message during live debugging, not anything in this code.
- Restored the legacy app's behavior of speaking every AI reply aloud automatically as soon as it finishes streaming (`ChatPage.tsx`) — the original React migration had kept only the Translate button's on-demand read-aloud and silently dropped the automatic one.
- A second occurrence of the "bare `string` serializes as `text/plain`, not JSON" bug (see T073's note on `EnableTwoFactor`/`IssueExternalLoginLinkTicket`) found in `AiController.Translate` — this one predates this session entirely (present since T038) and had been silently breaking the Translate feature this whole time: the frontend's `apiFetch` always calls `response.json()`, which threw `Unexpected token '<'...` trying to parse the raw HTML response body, an error nothing ever surfaced to a user. Fixed with `[Produces("application/json")]`, same as the other two; verified live afterward — translating a reply now round-trips correctly and appears in history.
