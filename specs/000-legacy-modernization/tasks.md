...
- `useVoiceRecognition.ts`'s error handling was completely silent (`onerror: () => setIsListening(false)`) — a permission denial, missing microphone, network error, or the browser never starting at all all looked identical to "nothing happened." Added error messages surfaced via a Snackbar in `ChatComposer.tsx`, plus a 15s watchdog for the "never even started" case (verified live to correctly catch a stuck/no-permission-decision state; the "stays quiet on a real working mic" path could not be validated in this sandbox — headless Chromium here never fires speech recognition's `onstart` event at all, granted permission or not).
