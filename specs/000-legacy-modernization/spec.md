...
- Q: The legacy app's scaffolded Identity UI supports linking an additional Google/Facebook login to an already-authenticated account from account settings (`Manage/ExternalLogins.cshtml.cs`'s `OnPostLinkLoginAsync`/`LinkLoginCallback`), separately from first-time social sign-in (FR-010). Is that linking capability in scope for this migration? → A: Yes, in scope for SPEC-000 as a minor FR completion (see FR-034), following the same precedent as FR-033 (chat rename/delete) — existing scaffolded functionality preserved, not a new capability.
- Q: `appsettings.json` is intentionally secret-free (per ADR-0001), and the CI/CD `deploy` job only publishes that file — nothing currently delivers real production secrets (JWT signing key, OpenAI key, SendGrid key, DB connection string) to the deployed app, and the `site4now.net` shared host has no environment-variable panel or secrets vault. What mechanism should close this gap? → A: A gitignored `appsettings.Production.json`, populated once by hand outside the CI/CD pipeline and pushed directly to the server; ASP.NET Core layers it over `appsettings.json` automatically under `ASPNETCORE_ENVIRONMENT=Production`, so secrets never touch git and the host needs no env-var feature it doesn't have.
- Q: Live testing surfaced that the chat sidebar and the actual conversation pane are completely disconnected — sending a message never creates/updates a sidebar entry, and clicking an existing entry loads nothing, because (as designed) no message/conversation content is ever persisted (FR-026). Should this remain as originally scoped (matching the legacy app's equally disconnected behavior), or should persisted conversation history be pulled into this migration now? → A: Override FR-026 for this one capability. Add a persisted `Messages` table and a ChatGPT-style experience — sending a message auto-creates and selects a chat if none is active, chat/translation/image-generation turns are all persisted, and selecting a chat from the sidebar loads its full history. See FR-036/FR-037 and § Key Entities. This is a deliberate, explicit override of FR-026's original constraint (not a silent scope change) — full persisted conversation *management* (multiple simultaneous views, search, sharing, etc.) beyond this ChatGPT-style baseline remains SPEC-002's job.
...
- **FR-034**: System MUST allow an authenticated user to link an additional external login provider (Google/Facebook) to their existing account from account settings, in addition to viewing and removing already-linked providers, preserving the legacy scaffolded Identity UI's `Manage/ExternalLogins` linking capability. This completes an existing entity's lifecycle (view/remove already implemented; add/link is the missing piece) rather than introducing a new business capability (see Assumptions).
- **FR-035**: System MUST have a defined mechanism for production secrets (JWT signing key, OpenAI API key, SendGrid API key, database connection string) to reach the deployed application without ever being committed to source control, given the `site4now.net` shared host has no environment-variable panel or secrets vault (unlike Azure App Service). This migration adopts a gitignored `appsettings.Production.json`, populated once by hand outside the CI/CD pipeline, layered automatically over `appsettings.json` by ASP.NET Core under `ASPNETCORE_ENVIRONMENT=Production`.
...
- **FR-026**: System MUST NOT introduce knowledge base, RAG/embeddings, AI memory, agents, MCP, prompt library, or billing changes as part of this migration. **Superseded in part by FR-036/FR-037** (2026-07-28 Clarifications): this requirement's original ban on a persisted message/conversation-content table no longer applies — a `Messages` table is now in scope, as an explicit, recorded override rather than a silent scope change. Full conversation *management* beyond the ChatGPT-style baseline in FR-036/FR-037 (multiple simultaneous views, search, sharing, etc.) remains SPEC-002's job.
- **FR-036**: System MUST persist every chat/translation/image-generation turn (user input and the AI's response) against the `UserChat` it belongs to, and MUST automatically create and select a new `UserChat` the moment a user sends their first message without one already selected — matching a ChatGPT-style "send starts a conversation" experience rather than requiring a separate, manual "create chat" step first.
- **FR-037**: System MUST allow a user to select any of their own saved chats and load its full persisted message history into the conversation view, in original chronological order.
...
- **UserChat** (existing, migrated) — the existing chat-title entity; migrated onto standard conventions (GUID key, audit columns, soft delete, concurrency token).
- **Message** (new, 2026-07-28 — FR-036/FR-037) — one turn of a `UserChat`'s persisted history: `Role` (User/Assistant), `Kind` (Text/Image/Translation), `Content` (chat text, image URL, or translated text), optional `SourceText` (the original prompt behind an Image/Translation-kind assistant turn), and the standard audit columns. Immutable once created (append-only) — no rename/edit. This is the one entity FR-026's original "no persisted message/conversation-content table" ban explicitly excluded; see FR-026's superseded note.
...
Explicitly **not** introduced in this migration: `MessageAttachments`, `KnowledgeBases`, `Documents`, `Embeddings`, `Agents`, `MCPServers`, `PromptTemplates`, `UserSubscriptions` — all reserved for the future specifications in [§ Future Specifications](#future-specifications). `Conversations`/`Messages` are no longer fully reserved — `Message` (above) is now in scope for the ChatGPT-style baseline in FR-036/FR-037; a richer `Conversations` concept (if ever distinct from `UserChat`) remains SPEC-002's job.
...
- [ ] Users can link an additional external login provider to their account, and view/remove linked providers (FR-034).
- [ ] Production secrets reach the deployed application via a gitignored `appsettings.Production.json` pushed outside CI/CD, never committed to source control (FR-035).
- [ ] Sending a message auto-creates and selects a chat when none is active, every chat/translation/image turn is persisted, and selecting a saved chat reloads its full history in order (FR-036, FR-037).
...
| Production secrets (FR-035) are delivered via a hand-pushed `appsettings.Production.json`, outside CI/CD | Deployment / Operational | No automated rotation or drift-detection for this file; a lost or stale copy silently breaks production. Acceptable for the current small-scale/single-instance deployment (see Assumptions), revisit if a secrets-vault-capable host is ever adopted. |
...
- **Superseded 2026-07-28**: conversation content is now persisted server-side (FR-036/FR-037) — see the Session 2026-07-28 Clarifications entry recording this explicit override of the original "content lives only in browser memory" assumption. Anything beyond the ChatGPT-style baseline shipped here (multiple simultaneous views, search, sharing, cross-device sync guarantees, etc.) remains SPEC-002's job.
...
- **External-login linking (FR-034) is an approved, minor exception to strict 1:1 preservation, matching FR-033's precedent.** The legacy app's scaffolded Identity UI already supports linking an additional Google/Facebook login to an authenticated account (`Manage/ExternalLogins.cshtml.cs`'s `OnPostLinkLoginAsync`/`LinkLoginCallback`), separately from first-time social sign-in (FR-010). Preserving that linking capability is treated as finishing an existing entity's lifecycle (view/remove already implemented; add/link is the missing piece), not as introducing a new business capability.
- **Production secret delivery (FR-035) uses a hand-pushed, gitignored `appsettings.Production.json`, not an environment-variable mechanism.** This is distinct from — and does not reopen — ADR-0001's accepted-risk deferral of rotating the *already-exposed legacy* secrets; FR-035 only concerns how the *new* application's required secrets reach production at all, since the `site4now.net` shared host has no environment-variable panel or secrets vault the CI/CD pipeline could otherwise populate.
