...
- **FR-001a**: Framing under FR-001 MUST be triggered only by a location being *deliberately established* — the user or the agent naming a place — and MUST NOT be re-triggered by continued reporting of the location already shown. Device geolocation keeps emitting fixes for the whole session (to detect permission revocation), each differing by metres of drift; treating those as new locations moved the camera off wherever the user had put it, at unpredictable intervals. Returning to the workspace MUST likewise re-frame nothing, since the place on screen has not changed. See the *Camera ownership* section of `src/AskLucy.Web/ClientApp/src/viewer/README.md`.
