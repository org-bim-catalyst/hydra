...
- [X] T003 [US2] In `ChatComposer.tsx`, extend the single-persistent-mic-element block's condition from `composerVisualState === 'empty' || composerVisualState === 'recording'` to also include `'typing'` (research.md Decision 2), and render the attachment button whenever `composerVisualState === 'empty' || composerVisualState === 'typing'` (not only `'empty'`), keeping the continuous-conversation-entry button restricted to `'empty'` only.
- [X] T004 [US2] In `ChatComposer.tsx`, ensure the Send button (currently gated on `composerVisualState === 'typing'` alone) continues to render for `'typing'`, now alongside attach+mic rather than as the row's only content.
- [X] T005 [US2] In `ChatComposer.test.tsx`, add tests: (a) typing renders attach + mic + Send together; (b) tapping mic while text is present starts a recording and, on finish, the transcript is appended after the existing typed text; (c) holding the mic while text is present behaves identically to hold-to-talk from the empty state; (d) the mic `IconButton` is the same DOM node across a press that starts in `'typing'` and transitions into `'recording'` (extend the existing specs/033 element-identity assertion pattern to cover this transition, not only empty→recording).
...
- [ ] T008a [P] [US3] In the same new `RecordingReviewControls.test.tsx`, add a `jest-axe` accessibility check (matching this codebase's established `.a11y.test.tsx` pattern, e.g. `MessageBubble.a11y.test.tsx`) covering both the `middle`-provided and `middle`-omitted render variants — required by constitution §7/§10's automated-a11y-check merge gate, and this is the component's first-ever test coverage of any kind, so there's no pre-existing a11y suite to rely on instead.
- [ ] T009 [US3] In `ChatComposer.test.tsx`, add/update a test asserting the tap-review row's DOM order is cancel → waveform → finish, and that both controls still call `onCancelRecording`/`onFinish` correctly (existing behavioral tests must continue passing, only order assertions are new).
