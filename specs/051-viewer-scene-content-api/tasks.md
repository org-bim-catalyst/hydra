...
- [X] T005a *(Amendment 2026-09-14, FR-008)* The reference point follows the active location: `viewer/session/anchorFollowsActiveLocation.ts` moves it on every active-location change; `SceneAnchor` gains `subscribe()`/`version` (notifying only on a real move); the map layer sets it only when unset, so recreation never moves it; `SiteBoundaryOverlay`, the solar building footprints and `ViewerEngine`'s loaded models re-position when it moves. Tests: `SceneAnchor.test.ts`, `anchorFollowsActiveLocation.test.ts`, `SiteBoundaryOverlay.test.tsx`
