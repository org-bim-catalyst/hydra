...
- [X] T051b *(Amendment 2026-09-14, FR-028)* Extensions keep running across a viewer unmount; `viewer/session/resetViewerSession.ts` stops them, unloads the map and clears the camera on sign-out. The map content id and camera move to `viewer/session/viewerSession.ts` so a remount re-attaches. `CameraAttitudeWidget` subscribes to `cameraChanged` once from `solarAnalysisExtension.start()`. Supersedes T051/T051a's unmount behaviour; tests in `ViewerSurface.test.tsx` and `CameraAttitudeWidget.test.tsx`
