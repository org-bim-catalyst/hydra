    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                ApplicationUser _user = await _userManager.FindByEmailAsync(info.Principal.FindFirstValue(ClaimTypes.Email));
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        if (result.Succeeded)
                        {
                            ...
                            if (_userManager.Options.SignIn.RequireConfirmedEmail)
                            {
                                ...
                                await _emailSender.SendEmailAsync(Input.Email, "Confirm your email", htmDoc.Text);
                            }
                ...
                {
                    var result = await _userManager.AddLoginAsync(_user, info);
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        if (_userManager.Options.SignIn.RequireConfirmedEmail)
                        {
                            ...
                            string html = System.IO.File.ReadAllText(htmlFilePath);
                            htmDoc.LoadHtml(html);
                            htmDoc.GetElementbyId("span-user-name").InnerHtml = _user.FirstName ?? string.Empty;
                            htmDoc.GetElementbyId("span-email-account").InnerHtml = _user.Email;
                            htmDoc.GetElementbyId("link-verify-email").SetAttributeValue("href", HtmlEncoder.Default.Encode(callbackUrl));
                            htmDoc.Save(htmlFilePath);
                            htmDoc.Load(htmlFilePath);
