...
/// </summary>
public sealed class StreamVoiceReplyCommandHandler(
    ISender mediator,
    ...
    ICurrentUserAccessor currentUser) : IStreamRequestHandler<StreamVoiceReplyCommand, VoiceReplyEvent>
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
            ...
            cancellationToken))
        {
            if (!string.IsNullOrEmpty(chunk.ContentDelta))
            {
                ...
                while (!ttsFailed && SentenceSegmenter.TryExtractSentence(sentenceBuffer, out var sentence))
                {
    ...
    private IAsyncEnumerable<VoiceReplyEvent> SynthesizeSentenceAsync(
        string sentence,
        VoiceSettingsDto settings,
        SequenceCounter sequenceCounter,
        string userId,
        CancellationToken cancellationToken) =>
        TextToSpeechStreamer.StreamAsync(
            textToSpeechProvider, healthRecorder, sentence, settings, sequenceCounter.Next, userId, cancellationToken);

    private Task<VoiceSettingsDto> ResolveVoiceSettingsAsync(string userId, string language, CancellationToken cancellationToken) =>
        TextToSpeechStreamer.ResolveVoiceSettingsAsync(textToSpeechProvider, voicePreferences, userId, language, cancellationToken);
}
