...
///
/// <para>
/// <b>Kinds offered.</b> Only <c>capability</c> and <c>followUp</c> are shown to the model as
/// choices. <c>flowVariant</c> exists in the domain model (FR-021a) but has nothing to point at
/// until specs/045 Phase 6 registers flow definitions — telling the model about a kind it can
/// never ground would just spend its attention on rows <see cref="Runtime.SuggestedActionGrounder"/>
/// always discards. <c>decline</c> is never requested from the model at all (FR-022).
/// </para>
/// </summary>
public static class SuggestedActionPrompt
{
        ...
        int maxSubstantiveActions)
    {
        ...
        builder.AppendLine(
            """{"question":"<what you ask before the list>","actions":[{"kind":"capability"|"followUp","key":"<capability key, capability only>","arguments":{},"text":"<composed instruction, followUp only>","label":"<short>","description":"<one line>"}]}""");
        builder.AppendLine();
        ...
        builder.AppendLine("Two kinds of row, composed for this specific situation, not from a fixed list:");
        builder.AppendLine(
        ...
        {
            builder.AppendLine(
                "No capability is worth suggesting right now. Only \"followUp\" rows are possible " +
                "this turn, and an empty \"actions\" array remains the right answer unless a " +
                "specific follow-up is genuinely useful.");
        }
        else
        {
