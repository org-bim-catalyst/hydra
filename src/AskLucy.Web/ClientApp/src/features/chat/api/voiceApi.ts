...
export async function* streamVoiceReply(
  ...
  if (!response.ok || !response.body) {
    ...
    throw new Error(
      problem?.detail ?? problem?.title ?? `Voice reply request failed with ${response.status}`,
    )
  }
...
/**
 * FR-006's "speak every AI reply aloud" — synthesizes speech for text that already exists
 * (unlike {@link streamVoiceReply}, which generates a new LLM reply and speaks it as it
 * streams). Same multiplexed-event shape and SSE-over-fetch parsing as `streamVoiceReply`.
 */
export async function* synthesizeSpeech(
  ...
  const response = await fetch(`${API_BASE_URL}/ai/voice/speak`, {
    method: 'POST',
    signal,
    headers: {
      'Content-Type': 'application/json',
  ...
  if (!response.ok || !response.body) {
    ...
    throw new Error(
      problem?.detail ??
        problem?.title ??
        `Speech synthesis request failed with ${response.status}`,
    )
  }
