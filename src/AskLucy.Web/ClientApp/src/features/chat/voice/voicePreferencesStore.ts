...
import { getVoicePreferences, saveVoicePreferences, type UserVoicePreference } from '../api/voiceApi'
    ...
    (set, get) => ({
      ...
      hydrateFromServer: async () => {
        try {
          ...
          set({ error: error instanceof Error ? error.message : 'Failed to load voice preferences.' })
        }
      },
      ...
      update: async (patch) => {
        ...
        try {
          ...
          set({ ...previous, error: error instanceof Error ? error.message : 'Failed to save voice preferences.' })
        }
      },
