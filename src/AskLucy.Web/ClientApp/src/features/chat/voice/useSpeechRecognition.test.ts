...
class FakeAudioWorkletNode {
  ...
  port: { postMessage: ReturnType<typeof vi.fn>; close: ReturnType<typeof vi.fn>; onmessage: ((event: { data: Float32Array }) => void) | null } = {
    postMessage: vi.fn(),
    close: vi.fn(),
    onmessage: null,
  }
...
describe('useSpeechRecognition', () => {
  ...
  it('connects on the first attempt and starts listening (FR-001/FR-002)', async () => {
    ...
    vi.mocked(createSttSession).mockResolvedValue({ token: 'tok-1', expiresAtUtc: new Date().toISOString() })
  ...
  it('surfaces partial and committed transcripts from the WebSocket (FR-001/FR-002)', async () => {
    ...
    act(() => {
      FakeWebSocket.instances[0].triggerMessage({ type: 'committed_transcript', text: 'Hello there' })
    })
