...
export function PrivacyPage() {
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 4 }, bgcolor: 'background.default', minHeight: '100%', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ flex: 1 }}>
        <PageHeader backTo={isAuthenticated ? '/chat' : '/login'} backLabel={isAuthenticated ? 'Back to chat' : 'Back to sign in'} title="Privacy Policy" />
        <Paper elevation={1} sx={{ maxWidth: 720, p: { xs: 3, sm: 4 } }}>
          <Stack spacing={3}>
            {policy && (
              <Typography variant="body2" color="text.secondary">
                Policy version {policy.version} — effective {new Date(policy.effectiveAtUtc).toLocaleDateString()}
              </Typography>
            )}
