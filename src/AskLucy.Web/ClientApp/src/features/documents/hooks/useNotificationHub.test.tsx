...
describe('useNotificationHub', () => {
  ...
  it('still surfaces latest/dismiss for a received notificationCreated event', async () => {
    const { result } = renderHook(() => useNotificationHub(), { wrapper })
    await act(async () => {
      await Promise.resolve()
    })
    ...
    const payload = {
      ...
      message: 'A document finished processing',
      createdAtUtc: new Date().toISOString(),
    }
    act(() => handlers['notificationCreated']?.(payload))
